...
const data = [
    ...
    },
    {
        ...
        src: '/home/backgroundtwo.png',
        title: "SEO Services",
        para: "Raise your brand's profile and achieve better placement in search engine results across Canada with our expert SEO services.        ",
    },
    {
        id: 5,
        src: '/home/backgroundthree.png',
...
<h2 className='md:text-4xl text-3xl text-center font-bold'>Our Expertise </h2>
<h3 className='md:text-xl text-lg pt-2 text-center font-medium'> Globally, we deliver logistics and freight solutions that are both dynamic and efficient.</h3>
...
<div className='grid md:grid-cols-3 items-center justify-between gap-12 md:py-16 py-8 md:px-0 px-5'>
    {data.map((item, e) => (
       ...
       <div className='child1 w-full h-[60%] '>
           <img src={item.src} className=' group-hover:block hidden imageHover'  />
       </div>
       <div className='child2 bg-white px-6  flex flex-col justify-center gap-2 absolute top-5 left-5 w-[90%] h-[90%]  rounded-lg shadow-lg '  key={e}>
           <h5 className='font-bold text-xl text-center'>{item.title}</h5>
           <p className='text-center  tracking-tighter pt-4'>{item.para}</p>
       </div>
   </div>
    ))}
